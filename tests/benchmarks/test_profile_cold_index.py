...
from benchmarks.profile_cold_index import _PhaseRecorder, _embedding_counters, _summarize_records
...
def test_phase_recorder_tracks_exclusive_time_counters_and_call_sizes() -> None:
    ...
    total = recorder.phase("total_index")
    ...
    assert total.exclusive_wall_ns + encode.exclusive_wall_ns == total.wall_ns
    ...
    assert recorder.nested_wall_ns == {"total_index>static_model_encode": 30}
...
def test_summary_reports_median_min_max_for_numeric_leaves_only() -> None:
    ...
    records = [
        {
            "repetition": 1,
            "total": {"wall_ns": 30, "process_cpu_ns": 9},
            "counts": {"chunks": 3, "proof": True},
            "sizes": [1, 2],
        },
        {
            "repetition": 2,
            "total": {"wall_ns": 10, "process_cpu_ns": 7},
            "counts": {"chunks": 1, "proof": True},
            "sizes": [2, 3],
        },
        {
            "repetition": 3,
            "total": {"wall_ns": 20, "process_cpu_ns": 8},
    ...
    assert summary["total"]["wall_ns"] == {"median": 20.0, "min": 10, "max": 30}
    assert summary["total"]["process_cpu_ns"] == {"median": 8.0, "min": 7, "max": 9}
